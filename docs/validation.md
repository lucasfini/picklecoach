...
- The checked-in GitHub Actions workflow runs the locked install, rejects high/critical dependency advisories, checks strict TypeScript, runs all 31 tests and Expo Doctor, verifies the generated iOS contract, and builds both release exports for pull requests and pushes to `main`.
...
Native build output contains warnings from Expo/React Native dependencies, including future Swift 6 concurrency warnings. No warning originated from `PickleCoachPose`, and the current Swift 5 build succeeds. A physical-device build also compiled through the custom pose module and reached final framework signing using the recovered local provisioning profile; the headless session could not authorize the login-keychain private key, so that build was not installed and is not counted as runtime evidence.
