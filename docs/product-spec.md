...
- The live camera contains the complete portrait 9:16 recorded frame instead of cropping it to fill a taller iPhone screen, so placement guidance matches review playback.
...
- Returning to a Pose Lab case shows its latest aggregate tracking summary even though the associated raw recording has already been discarded.
- Leaving a Pose Lab run with a pending visual review requires explicit confirmation because its temporary recording cannot be restored later.
